...
async function debugRegistrationFlow() {
  try {
    console.log('🐛 DEBUG: Starting registration flow debug...');
    ...
    console.log('📦 tempUsers found:', tempUsersSnapshot.size);
    tempUsersSnapshot.forEach(doc => {
      console.log('   -', doc.id, '=>', doc.data());
    });
    
    // 2. Check what's in verificationCodes
    console.log('🔍 Checking verificationCodes collection...');
    const verificationSnapshot = await getDocs(collection(db, "verificationCodes"));
    console.log('📦 verificationCodes found:', verificationSnapshot.size);
...
async function testAuthCreation() {
  try {
    ...
    console.log('🧪 Testing Auth creation:', testEmail);
    ...
    console.error('❌ Auth creation test failed:', error);
    throw error;
  }
}
